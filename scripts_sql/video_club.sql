...
CREATE TABLE IF NOT EXISTS `videoclub`.`reservaciones` (
  ...
  INDEX `fk_reservaciones_copias_idx` (`copias_id_copia` ASC),
  CONSTRAINT `fk_reservaciones_clientes`
    FOREIGN KEY (`clientes_id_cliente`)
    REFERENCES `videoclub`.`clientes` (`id_cliente`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_reservaciones_copias`
...
-- -----------------------------------------------------
-- Placeholder table for view `videoclub`.`disponibilidad_peliculas`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `videoclub`.`disponibilidad_peliculas` (`id_copia` INT, `id_pelicula` INT, `nombre` INT, `duracion` INT, `genero` INT, `disponibilidad` INT, `precio_alquiler` INT, `num_copia` INT);

-- -----------------------------------------------------
-- View `videoclub`.`disponibilidad_peliculas`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `videoclub`.`disponibilidad_peliculas`;
USE `videoclub`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `videoclub`.`disponibilidad_peliculas` AS select `cop`.`id_copia` AS `id_copia`,`pel`.`id_pelicula` AS `id_pelicula`,`pel`.`nombre` AS `nombre`,`pel`.`duracion` AS `duracion`,`gen`.`nombre` AS `genero`,`cop`.`disponibilidad` AS `disponibilidad`,`pel`.`precio_alquiler` AS `precio_alquiler`,(select count(`videoclub`.`copias`.`id_copia`) from `videoclub`.`copias` where ((`videoclub`.`copias`.`peliculas_id_pelicula` = `pel`.`id_pelicula`) and (`videoclub`.`copias`.`disponibilidad` = 'disponible'))) AS `num_copia` from ((`videoclub`.`peliculas` `pel` join `videoclub`.`generos` `gen` on((`pel`.`generos_id_genero` = `gen`.`id_genero`))) join `videoclub`.`copias` `cop` on((`pel`.`id_pelicula` = `cop`.`peliculas_id_pelicula`)));
